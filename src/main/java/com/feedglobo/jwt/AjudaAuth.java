...
/**
 * Classe com metodos de suporte a autenticação. Ela é reponsável pro gerar um
 * gerar e verificar um jwt
 *
 */
public class AjudaAuth {
     ...
     * carga útil (por exemplo, userId para identificar o usuário). A chave de
     * assinatura é secreta. Isso garante que o token seja autêntico e não tenha
     * sido modificado. O uso de um jwt elimina a necessidade de armazenar
    ...
    public static String createJsonWebToken(String idUsuario, int diasDuracao) {
        ...
        token.setExpiration(new Instant(cal.getTimeInMillis() + 1000L * 60L * 60L * 24L * diasDuracao));
    ...
    /**
     * Verifica a validade do token da web json e extrai o id do usuário e outras
     * informações dele.
     *
     * @param token
     * @return
