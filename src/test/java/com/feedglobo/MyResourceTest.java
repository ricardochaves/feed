...
public class MyResourceTest {
    ...
    public void setUp() throws Exception {
        ...
        Client c = ClientBuilder
                .newClient(new ClientConfig().register(FeedResource.class).register(new AbstractBinder() {
                    @Override
    ...
    public void testeXMLConvertGetArrayDeItensSucesso() {
        ...
        assertEquals(listaitens.getJSONObject(0).getString("title").trim(),
                "Ford lança Ranger Sportrac por R$ 159.990");
        assertEquals(listaitens.getJSONObject(1).getString("title").trim(),
                "Novo Porsche Cayenne tem primeiras fotos vazadas antes do lançamento");
        assertEquals(listaitens.getJSONObject(0).getString("link").trim(),
                "http://revistaautoesporte.globo.com/Noticias/noticia/2017/08/ford-lanca-ranger-sportrac-por-r-159990.html");
        assertEquals(listaitens.getJSONObject(1).getString("link").trim(),
                "http://revistaautoesporte.globo.com/Noticias/noticia/2017/08/novo-porsche-cayenne-tem-primeiras-fotos-vazadas-antes-do-lancamento.html");
    }
    ...
    public void testConverteItemsToFeedItem() {
        ...
        assertEquals(resultado.getJSONObject(0).getJSONObject("item").getString("title").trim(),
                "Ford lança Ranger Sportrac por R$ 159.990");
        assertEquals(resultado.getJSONObject(1).getJSONObject("item").getString("title").trim(),
                "Novo Porsche Cayenne tem primeiras fotos vazadas antes do lançamento");
        assertEquals(resultado.getJSONObject(0).getJSONObject("item").getString("link").trim(),
                "http://revistaautoesporte.globo.com/Noticias/noticia/2017/08/ford-lanca-ranger-sportrac-por-r-159990.html");
        assertEquals(resultado.getJSONObject(1).getJSONObject("item").getString("link").trim(),
                "http://revistaautoesporte.globo.com/Noticias/noticia/2017/08/novo-porsche-cayenne-tem-primeiras-fotos-vazadas-antes-do-lancamento.html");
    ...
    private String GetXMLTest() {
        return "\r\n" + "<?xml version=\"1.0\" encoding=\"UTF-8\" ?>\r\n"
                ...
                + "  </item>\r\n" + "\r\n" + "</channel>\r\n" + "</rss>\r\n" + "";
        // return (new
        // XMLConvert()).downloadAsString("http://revistaautoesporte.globo.com/rss/ultimas/feed.xml");
    }

    /**
     * Testando o Resource apenas, com a injeção de depência usada aqui é possível
     * testar o metodo sem precisar baixar xml nem nada do tipo.
     */
    @Test
